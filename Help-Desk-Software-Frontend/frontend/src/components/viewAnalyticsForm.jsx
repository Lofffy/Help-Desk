...
function ViewAnalyticsForm() {
  ...
  Average Rating: {analyticsCharts.agentAvgRates && analyticsCharts.agentAvgRates[0] ? analyticsCharts.agentAvgRates[0].avgRate : 'N/A'} From{" "}
  {analyticsCharts.agentAvgRates && analyticsCharts.agentAvgRates[0] ? analyticsCharts.agentAvgRates[0].ratedTickets : 'N/A'} Tickets
</Typography>
        <div
          style={{
            display: "flex",
...
>
  {
    (analyticsCharts.categoryCounts || []).map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)
  }
</Pie>
            <Tooltip />
            <Legend />
